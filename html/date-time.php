...
<body>
    <p>
        ...
        //現在を基準として１週間後の日時のUNIXスタンプを出力する
        echo strtotime('+1 week') . '<br>';

        //現在を基準として３年前の日時を指定したフォーマットで出力する
    ...
    <p>
        ...
        //インスタンス$data_timeの日時を特定のフォーマットで出力する
        echo $date_time->format('Y年n月j日H時i分s秒') . '<br>';

        //現在の日時のインスタンスを作成する
        $now = new DateTime();
        //diff()メソッドで２つのインスタンスの日時の差を取得し、変数$intervalに代入する(interval=間隔)
        $interval = $now->diff($date_time);
        //取得した日時の差を特定のフォーマットで出力する
        echo $interval->format('%y年と%m月と%d日の差があります。総日数は%a日です。') . '<br>';
        //加算・減産してもインスタンス$nowの日時が変わらないようにDateTimeImmutableクラスを使って作成する
        $now = new DateTimeImmutable();
        //現在の日時を1年に加算し、変数$addに代入する
        $add = $now->modify('+1 year');
        //現在の日時から3日を減算し、変数$subに代入する（sub=「減算」を意味するsubstractionの略）
        $sub = $now->modify('-3 day');
        //加算・減算した日時を特定のフォーマットで出力する
        echo $add->format('現在から1年後はY年n月j日H時i分s秒です。') . '<br>';
        echo $sub->format('現在から3日後はY年n月j日H時i分s秒です。') . '<br>';
        ?>
    </p>    
</body>
</html>